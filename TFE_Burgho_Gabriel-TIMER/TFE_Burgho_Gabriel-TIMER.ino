...
void setup() {
  ...
  //fast_samd21_tc4_configure(100);//us
...
{//entree ADC remplir buffer
  ...
  Serial.println(buffer_parler[currentIndex]);
  currentIndex++;
  ...
  if (!digitalRead(pin_CS)) 
    {
      ...
      if (mode)
        {//active TC ADC
          ...
          if(digitalRead(pin_CS)) desactive_TC();
        } else {
          //active TC DAC
